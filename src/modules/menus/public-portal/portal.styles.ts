...
@media(min-width:800px){
  ...
  .pr-online-chip{
    ...
    background:var(--green-dim);border:1px solid rgba(22,163,74,.15);
    border-radius:5px;padding:3px 9px
  }
  .pr-online-chip::before{content:'';width:5px;height:5px;border-radius:50%;background:var(--green)}
...
.hm-cover-overlay{
  ...
  background:linear-gradient(to top,rgba(0,0,0,.48) 0%,rgba(0,0,0,.04) 60%,transparent 100%);
}
.hm-cover-content{
  position:absolute;bottom:14px;left:18px;right:18px;z-index:1
}
.hm-cover-name{
...
.hm-profile-hours svg{width:12px;height:12px;flex-shrink:0}
...
.hm-pstats{display:flex;align-items:center;gap:14px;padding-top:2px}
.hm-pstat{
  display:flex;align-items:center;gap:5px;
  font-size:12px;font-weight:500;color:var(--soft)
}
.hm-pstat-lbl{color:var(--dim)}
.hm-pstat-svc{}
.hm-pstat-star{color:var(--soft)}
.hm-pstat-rev{}
...
.btn-primary{
  ...
  border:none;border-radius:var(--r);padding:12px 20px;cursor:pointer;
  transition:opacity .15s,transform .12s;text-decoration:none;letter-spacing:-.01em;
  -webkit-tap-highlight-color:transparent
}
.btn-primary:hover{opacity:.88}
.btn-primary:focus-visible{outline:3px solid var(--primary-glow);outline-offset:2px}
...
.rdash-title{font-size:18px;font-weight:700;color:var(--text);letter-spacing:-.04em}
...
/* Hoy */
.mc-today{background:#3B82F6}
.mc-today .mc-num{color:#fff;font-weight:700}
.mc-today.mc-avail{background:#3B82F6}
...
.qp-inp{
  ...
  background:var(--panel);border:none;border-radius:11px;
  color:var(--text);font-family:inherit;font-size:14px;
  padding:11px 14px;margin-bottom:10px;outline:none;
  -webkit-appearance:none;appearance:none;
  transition:box-shadow .15s
}
.qp-inp:focus{box-shadow:0 0 0 3px var(--primary-glow)}
...
.cal-cell.cal-today{background:#3B82F6;color:#fff;font-weight:700}
...
.bk-inp:focus{box-shadow:0 0 0 3px var(--primary-glow)}
...
.mob-badge::before{content:'';width:5px;height:5px;border-radius:50%;background:var(--green)}
.mob-info-row{
...
.gate-biz{font-size:17px;font-weight:700;color:var(--text);letter-spacing:-.03em}
...
.hm-topbar-left{}
.hm-topbar-greet{font-size:20px;font-weight:700;color:var(--text);letter-spacing:-.04em;line-height:1.15}
...
.hm-stat-val{font-size:22px;font-weight:700;color:var(--text);letter-spacing:-.04em;line-height:1;font-variant-numeric:tabular-nums}
...
.hm-main{display:flex;flex-direction:column;gap:0;padding:0 0 40px}
...
.hm-card-title{font-size:12.5px;font-weight:600;color:var(--text);letter-spacing:-.01em}
...
.hm-card-cal,
.hm-card-svc,
.hm-card-reviews{background:#fff}
...
.pdp-photo-single{width:100%;aspect-ratio:4/3;background:#000;overflow:hidden}
.pdp-photo-single img{width:100%;height:100%;object-fit:contain;display:block}
.pdp-no-photo{width:100%;height:160px}
.pdp-gallery{width:100%;aspect-ratio:4/3;background:#000;display:flex;overflow-x:auto;scroll-snap-type:x mandatory;scrollbar-width:none;-webkit-overflow-scrolling:touch}
.pdp-gallery::-webkit-scrollbar{display:none}
.pdp-gallery-img,.pdp-gallery-main{flex-shrink:0;width:100%;height:100%;object-fit:contain;display:block;scroll-snap-align:start}
...
.gal-lb-wrap{position:relative;display:flex;flex-direction:column;flex:1;min-height:0}
...
.hm-gal-card img,.hm-gal-card-empty{width:100%;height:220px;object-fit:cover;display:block}
...
@media(min-width:800px){
  ...
  .hm-left-col{
    ...
    gap:0;overflow-y:auto;min-height:0
  }
  .hm-right-col{
    flex:4;flex-shrink:0;display:flex;flex-direction:column;
    gap:0;overflow:hidden;min-height:0;order:0
  }
  .hm-section{border-top:1px solid var(--border-inner)}
  .hm-section:first-child{border-top:none}
  .hm-sec-hdr{padding:16px 4px 12px}
  .hm-card-cal{flex:1}
